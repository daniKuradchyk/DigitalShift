...
type Errors = { name?: string; email?: string; objective?: string };
    ...
    <section id="contacto" aria-labelledby="contact-title" className="py-16 sm:py-20">
      <Container>
        <div className="grid gap-8 lg:grid-cols-2 items-start">
          ...
          <form onSubmit={onSubmit} className="rounded-2xl border border-slate-200 p-6 shadow-sm bg-white" aria-describedby="rgpd-note status-msg">
            <div className="grid gap-4 sm:grid-cols-2">
              <div className="sm:col-span-1">
                <label htmlFor="name" className="block text-sm font-medium text-slate-900">Nombre*</label>
                <input id="name" name="name" type="text" required autoComplete="name" aria-invalid={!!errors.name} aria-describedby={errors.name ? "name-error" : undefined} className={`mt-1 w-full rounded-xl border px-3 py-2 ${errors.name ? "border-red-500" : "border-slate-300"}`} />
              ...
              <div className="sm:col-span-1">
                ...
                <input id="email" name="email" type="email" required autoComplete="email" aria-invalid={!!errors.email} aria-describedby={errors.email ? "email-error" : undefined} className={`mt-1 w-full rounded-xl border px-3 py-2 ${errors.email ? "border-red-500" : "border-slate-300"}`} />
              ...
              <div className="sm:col-span-1">
                ...
                <input id="company" name="company" type="text" autoComplete="organization" className="mt-1 w-full rounded-xl border border-slate-300 px-3 py-2" />
              </div>
              <div className="sm:col-span-1">
            ...
            <div className="mt-5 flex items-center gap-3">
              ...
              <p id="status-msg" aria-live="polite" className={`${status === "error" ? "text-red-700" : status === "ok" ? "text-emerald-700" : "text-slate-600"}`}>
                {status !== "idle" ? message : ""}
              </p>
            </div>
