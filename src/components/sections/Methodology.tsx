...
export default function Methodology() {
  ...
  return (
    <section
      ...
      className="relative scroll-mt-24 py-16 sm:py-20 md:py-28 lg:py-32"
    >
      ...
      <Container className="relative">
        ...
        >
          ...
          <h2 id="method-title" className="text-h2 mt-4 mb-4" style={{ color: "var(--text-primary)" }}>
            Del diagnóstico al{" "}
            <span className="gradient-text-static">resultado medible</span>
          </h2>
          <p className="text-sm sm:text-base md:text-lg leading-relaxed max-w-xl mx-auto" style={{ color: "var(--text-secondary)" }}>
        ...
        <div className="grid lg:grid-cols-2 gap-8 sm:gap-10 lg:gap-14 items-center">
          ...
          >
            ...
            <div className="absolute -inset-4 bg-blue-500/[0.04] rounded-3xl blur-[60px] -z-10" />
          </motion.div>
          ...
          <div className="space-y-3 sm:space-y-4 order-1 lg:order-2">
            {STEPS.map((step, i) => (
              <motion.div
                key={step.step}
                className="group relative flex gap-3 sm:gap-4 p-3 sm:p-4 rounded-xl transition-all duration-400 hover:bg-blue-500/[0.04] border border-transparent hover:border-blue-500/10"
                initial={{ opacity: 0, x: 20 }}
                animate={inView ? { opacity: 1, x: 0 } : {}}
                transition={{ duration: 0.6, delay: 0.2 + i * 0.1, ease: EASE }}
              >
                <div className="flex-shrink-0">
                  <div className="w-10 h-10 sm:w-12 sm:h-12 rounded-xl bg-blue-500/10 border border-blue-500/20 flex items-center justify-center text-blue-400 group-hover:bg-blue-500/15 group-hover:border-blue-500/30 transition-all">
                    {step.icon}
                  </div>
                </div>
                <div className="min-w-0">
                  <div className="flex items-center gap-2 sm:gap-3 mb-1">
                    <span className="text-[10px] sm:text-xs font-mono font-bold tracking-widest" style={{ color: "rgba(91,141,239,0.5)" }}>{step.step}</span>
                    <div className="h-px flex-1 bg-blue-500/10" />
                  </div>
                  <h3 className="text-base sm:text-lg font-bold mb-0.5 group-hover:text-blue-400 transition-colors" style={{ color: "var(--text-primary)" }}>
                    {step.title}
                  </h3>
