...
const HUB_COLORS = ["65,105,225", "91,141,239", "100,149,237", "30,80,200", "133,162,255"];
...
function runCanvas(canvas: HTMLCanvasElement): () => void {
  ...
  type GSeg = [number, number, number, number, number];
  const base: Seg[] = [];
  const glow: GSeg[] = [];
  const hubseg: Seg[] = [];
  ...
  function draw() {
    ...
    if (base.length > 0) {
      ctx.beginPath();
      ctx.strokeStyle = "rgba(133,162,255,0.10)";
      ctx.lineWidth = 0.5;
      for (const [x1, y1, x2, y2] of base) { ctx.moveTo(x1, y1); ctx.lineTo(x2, y2); }
      ctx.stroke();
    }
    ...
    if (hubseg.length > 0) {
      ...
      ctx.lineWidth = 0.7;
      for (const [x1, y1, x2, y2] of hubseg) { ctx.moveTo(x1, y1); ctx.lineTo(x2, y2); }
      ctx.stroke();
    }
    ...
    for (const [x1, y1, x2, y2, alpha] of glow) {
      ...
      ctx.strokeStyle = `rgba(91,141,239,${alpha * 0.22})`;
    ...
    for (let i = 0; i < pts.length; i++) {
      ...
      if (p.hub) {
        ...
        const hc = mg > 0.1 ? "91,141,239" : HUB_COLORS[i % HUB_COLORS.length];
      ...
      if (mg > 0.15) {
        ...
        eg.addColorStop(0, `rgba(91,141,239,${mg * 0.55})`);
        eg.addColorStop(1, `rgba(91,141,239,0)`);
        ctx.fillStyle = eg;
        ctx.beginPath(); ctx.arc(p.x, py, er, 0, Math.PI * 2); ctx.fill();
      }

      const clr = mg > 0.30 ? "91,141,239" : p.hub ? "65,105,225" : "133,162,255";
    ...
    pulses = pulses.filter((pulse) => {
      ...
      for (let k = 1; k <= 5; k++) {
        ...
        ctx.fillStyle = `rgba(91,141,239,${ta})`;
      ...
      gp.addColorStop(0, `rgba(133,162,255,${fad * 0.90})`);
      gp.addColorStop(0.4, `rgba(91,141,239,${fad * 0.45})`);
      gp.addColorStop(1, `rgba(91,141,239,0)`);
      ctx.fillStyle = gp;
      ctx.beginPath(); ctx.arc(x, y, 6, 0, Math.PI * 2); ctx.fill();

      ctx.fillStyle = `rgba(200,220,255,${fad * 0.85})`;
